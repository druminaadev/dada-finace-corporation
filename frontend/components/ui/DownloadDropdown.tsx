  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        style={{ 
          color: open ? '#462C7D' : '#6b7280',
          background: open ? 'rgba(70, 44, 125, 0.12)' : 'transparent',
          transform: open ? 'scale(1.05)' : 'scale(1)'
        }}
        onMouseEnter={e => {
          if (!open) {
            e.currentTarget.style.background = 'rgba(70, 44, 125, 0.08)'
      ...
      {open && (
        <div 
          className="absolute right-0 z-[9999] mt-2 w-56 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200"
          style={{ 
            background: 'var(--bg)', 
        ...
        >
          <div className="py-2 max-h-96 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
            <div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)', opacity: 0.7 }}>
              Download Documents
            </div>
            {DOCUMENTS.map((doc, idx) => (
              <button 
                ...
                className="w-full text-left px-3 py-2.5 text-sm flex items-center gap-3 transition-all duration-150"
                style={{ 
              ...
              >
                <div className="p-1.5 rounded-lg" style={{ background: 'rgba(70, 44, 125, 0.1)' }}>
                  <FileText size={14} style={{ color: '#462C7D' }} />
                </div>
                <span className="flex-1">{doc.label}</span>
                <Download size={12} style={{ color: 'var(--text-secondary)', opacity: 0.5 }} />
              </button>
            ))}
          </div>
        </div>
