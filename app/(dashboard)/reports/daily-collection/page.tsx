...
export default function DailyCollectionPage() {
  ...
  const handleExport = (formatType: string) => {
    ...
    if (formatType === 'csv') exportCSV(dataToExport, `Daily_Collection_${date}`)
    else if (formatType === 'excel') exportExcel(dataToExport, `Daily_Collection_${date}`)
    else if (formatType === 'pdf') exportPDF(dataToExport, `Daily Collection Report - ${date}`)
    else if (formatType === 'print') printTable(dataToExport, `Daily Collection Report - ${date}`)
  }
            ...
            {collected.map(e => {
              ...
              return (
                <div key={e.id} className="mx-3 my-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-4">
                  <div className="font-semibold text-[15px] leading-snug mb-1" style={{ color: 'var(--text-primary)' }}>{customer?.name ?? '—'}</div>
                  <div className="text-xs mb-3" style={{ color: 'var(--accent)' }}>{loan?.loanNo ?? '—'}</div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                    {([['EMI #', `#${e.instNo}`], ['Amount', fmt(e.paidAmount ?? 0)], ['Mode', e.paymentMode ?? '—'], ['Collected By', emp?.name ?? '—']] as [string, string][]).map(([label, value]) => (
                      <div key={label}>
