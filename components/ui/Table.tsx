...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function MobileCard<T extends Record<string, any>>({ row, columns }: { row: T; columns: Column<T>[] }) {
  const actionCol = columns.find(c => c.key === 'actions')
  const titleCol = columns.find(c => c.key !== 'actions' && c.key !== 'id' && c.key !== '#')
  const detailCols = columns.filter(c => c.key !== 'actions' && c.key !== titleCol?.key)
  return (
    <div className="mx-3 my-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-4">
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="font-semibold text-slate-800 dark:text-slate-100 text-[15px] leading-snug truncate flex-1 min-w-0">
          {titleCol ? (titleCol.accessor ? titleCol.accessor(row) : String(row[titleCol.key] ?? '—')) : '—'}
        </div>
        {actionCol && (
          <div className="shrink-0">{actionCol.accessor ? actionCol.accessor(row) : null}</div>
        )}
      </div>
    ...
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
      ...
      <div className="block sm:hidden">
        {paginated.length === 0
          ...
          : <div className="py-1">{paginated.map((row, i) => <MobileCard key={i} row={row} columns={columns} />)}</div>
        }
      </div>
