              ...
              return (
                <div key={mode} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 shadow-sm">
                  ...
                  <div className="mt-4 h-2 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden">
                    <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 via-sky-500 to-[var(--primary)]" style={{ width: `${share}%` }} />
                  </div>
          ...
          mobileCard={(row: PaymentRow) => (
            <div className="mx-3 my-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-4">
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-bold shrink-0 ${modeTone[row.paymentMode] ?? modeTone.Cash}`}>
                  {row.paymentMode}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                {([
                  ['Amount Paid', row.amountFmt],
                  ['Payment Date', row.paidDateFmt],
                ] as [string, string][]).map(([label, value]) => (
                  <div key={label}>
                    <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">{label}</div>
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">{value}</div>
                  </div>
                ))}
              </div>
            </div>
