          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4 shadow-sm">
              ...
              <div className="mt-4 h-2.5 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 via-sky-500 to-[var(--primary)]" style={{ width: `${completionRate}%` }} />
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-sm">
              ...
              <div className="block sm:hidden py-1">
                {loanEmis.map(emi => (
                  <div key={emi.id} className="mx-3 my-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        <div className="font-semibold text-[15px]" style={{ color: 'var(--text-primary)' }}>EMI #{emi.instNo}</div>
                        <div className="text-xs mt-0.5 text-slate-500 dark:text-slate-400">{format(parseISO(emi.dueDate), 'dd MMM yyyy')}</div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {(emi.status === 'overdue' || emi.status === 'upcoming') && (
                          <Button size="sm" onClick={() => openCollect(emi)}>Collect</Button>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-2">
                      {([
                        ['EMI Amount', fmt(emi.emiAmount)],
                        ['Principal', fmt(emi.principal)],
                        ['Interest', fmt(emi.interest)],
                        ['Outstanding', fmt(emi.outstanding)],
                        ['Paid Date', emi.paidDate ? format(parseISO(emi.paidDate), 'dd/MM/yyyy') : '—'],
                      ] as [string, string][]).map(([label, value]) => (
                        <div key={label}>
                          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">{label}</div>
                          <div className="text-xs font-semibold mt-0.5 text-slate-700 dark:text-slate-300">
                            {value}
                            {label === 'EMI Amount' && emi.penaltyAmount ? <span className="ml-1 text-rose-500">+{fmt(emi.penaltyAmount)}</span> : null}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {loanEmis.map((emi, i) => (
                      <tr key={emi.id} className={`border-b border-slate-100 dark:border-slate-700 hover:bg-[var(--background)]/50 dark:hover:bg-[var(--primary)]/10 transition-colors ${i % 2 === 1 ? 'bg-slate-50/30 dark:bg-slate-700/10' : ''}`}>
